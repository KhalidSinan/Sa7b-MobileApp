...
part '../../states/add_new_post_state.dart';
part 'add_new_post_cubit.freezed.dart';

class AddNewPostCubit extends Cubit<AddNewPostState> {
  final AddNewPostRepo _addNewPostRepo;
  AddNewPostCubit(this._addNewPostRepo) : super(AddNewPostState.initial());
  ...
  void addNewPost({
    ...
    try {
      ...
      final response = await _addNewPostRepo.addNewPost(request);
      emit(AddNewPostState.success(response));
    } catch (e) {
      emit(AddNewPostState.error(error: e.toString()));
    }
  }
  ...
  Future<void> pickImage() async {
    ...
    if (pickedFile != null) {
      ...
      emit(const AddNewPostState.imagePicked());
    }
  }
  ...
  Future<void> pickVideo() async {
    ...
    if (pickedFile != null) {
      ...
      emit(const AddNewPostState.videoPicked());
    }
  }
  ...
  Future<void> pickPdf() async {
    ...
    if (result != null && result.files.single.path != null) {
      ...
      emit(const AddNewPostState.pdfPicked());
    }
  }
  ...
  void clearMedia() {
    ...
    emit(const AddNewPostState.mediaCleared());
  }
